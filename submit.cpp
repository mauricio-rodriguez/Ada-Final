...
dataType S(vector<dataType> &p,dataType l, dataType r){
    if(l==0) return p[r];
    return p[r] - p[l-1];
}

dataType S2(vector<dataType> &p2,dataType l, dataType r){
    if(l==0) return p2[r];
    return p2[r] - p2[l-1];
}

dataType getCost(vector<dataType> &p, vector<dataType> &p2, dataType i, dataType j){
    dataType m = (i+j)/2;
    return S2(p2,i,m) - (i+1)*S(p,i,m) + (j+1)*S(p,m+1,j) - S2(p2,m+1,j);
}

dataType minPartition(Matrix &M,dataType n,vector<dataType> A,dataType k){
    //get Sum of prefix 
